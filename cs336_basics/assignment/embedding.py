...
class Embedding(nn.Module):
    def __init__(
        self, num_embeddings: int, embedding_dim: int, device: torch.device = None, dtype: torch.dtype = None
    ) -> None:
        """
    ...
    def forward(self, token_ids: torch.LongTensor) -> torch.Tensor:
        ...
        embedding = torch.empty(
            batch_size, seq_length, self.embedding_dim, device=token_ids.device, dtype=self.weight.dtype
        )
