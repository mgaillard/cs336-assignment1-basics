...
def create_rms_pre_norm(
    ...
    if rms_norm_type == "pre-norm":
        return nn.RMSNorm(normalized_shape, eps=eps, device=device, dtype=dtype)
    else:  # "post-norm" or "none"
        return nn.Identity()

def create_rms_post_norm(
    rms_norm_type: RMSNormType,
    normalized_shape: list,
    eps: float,
    device: torch.device | None = None,
    ...
    Returns:
    - nn.Module RMSNorm layer if post-norm, Identity layer otherwise
    """
    if rms_norm_type == "post-norm":
        return nn.RMSNorm(normalized_shape, eps=eps, device=device, dtype=dtype)
    else:  # "pre-norm" or "none"
        return nn.Identity()
